...
def parse_args():
    ...
    # 1. 核心参数 (加入了 cloudfront)
    parser.add_argument('--mode', choices=['beginner', 'normal', 'proxy', 'cloudfront'], required=True,
                       help='运行模式: beginner(小白), normal(常规), proxy(反代), cloudfront(CFT)')
    
    # 2. 基础参数
    parser.add_argument('--ipv6', action='store_true', help='使用 IPv6')
    parser.add_argument('--count', type=int, default=10, help='测试IP数量')
    parser.add_argument('--speed', type=float, default=1.0, help='下载速度下限 MB/s')
    parser.add_argument('--delay', type=int, default=1000, help='延迟上限 ms')
    parser.add_argument('--thread', type=int, default=200, help='延迟测速线程数')
    
    # 3. 模式专用参数
    parser.add_argument('--region', type=str, help='地区码 (常规模式)')
    parser.add_argument('--csv', type=str, default='result.csv', help='CSV路径 (反代模式)')
    # CFT 专用
    parser.add_argument('--url', type=str, help='测速文件地址 (CFT模式)')
    parser.add_argument('--ip-file', type=str, help='IP列表文件路径 (CFT模式)')
    
    # 4. 上传与结果处理
    parser.add_argument('--upload', choices=['api', 'github', 'none'], default='none', help='上传方式')
    parser.add_argument('--upload-count', type=int, default=10, help='上传数量')
    parser.add_argument('--clear', action='store_true', help='上传前清空 (仅API模式)')
    
    # 5. Cloudflare API / DNS 参数 (新增了 zone-id 等)
    parser.add_argument('--worker-domain', type=str, help='Worker域名 (Workers API)')
    parser.add_argument('--uuid', type=str, help='UUID或路径 (Workers API)')
    parser.add_argument('--zone-id', type=str, help='Zone ID (DNS API)')
    parser.add_argument('--api-token', type=str, help='API Token (DNS API)')
    parser.add_argument('--record-name', type=str, help='完整域名 (DNS API)')
    
    # 6. GitHub 参数
    parser.add_argument('--repo', type=str, help='仓库 owner/repo')
    parser.add_argument('--token', type=str, help='GitHub Token')
    parser.add_argument('--file-path', type=str, default='cloudflare_ips.txt', help='仓库内文件路径')
    
    return parser.parse_args()

def run_with_args(args):
    """使用命令行参数运行 (修复：运行前清理旧结果，防止脏数据上传)"""
    if sys.platform == "win32":
        try:
            import codecs
    ...
    # -------------------------------------------------
    # CloudFront (CFT) 命令行逻辑
    # -------------------------------------------------
    if args.mode == 'cloudfront':
        print(f"\n[CloudFront 优选模式]")
        
        # 1. 准备参数
        target_ip_file = args.ip_file
        target_url = args.url or "https://images-assets.nasa.gov/video/First-8K-Video-from-Space/First-8K-Video-from-Space~orig.mp4"
        cli_ip_ver = "--ipv6" if args.ipv6 else ""
        
        # 自动下载文件逻辑
        if not target_ip_file:
            target_ip_file = "cft_ipv6.txt" if args.ipv6 else "cft_ipv4.txt"
            if not os.path.exists(target_ip_file):
                print(f"文件 {target_ip_file} 不存在，尝试自动下载...")
                dl_url = "https://raw.githubusercontent.com/astemo6/yx-tools/main/CFT/ipv6.txt" if args.ipv6 else "https://raw.githubusercontent.com/astemo6/yx-tools/main/CFT/ipv4.txt"
                download_file(dl_url, target_ip_file)
        
        if not os.path.exists(target_ip_file):
            print(f"❌ 错误: 找不到有效 IP 文件: {target_ip_file}")
            return 1

        # 2. 运行测速
        if sys.platform == "win32": cmd = [exec_name]
        else: cmd = [f"./{exec_name}"]
        ...
        # 🛡️ 关键修复：运行前强制删除旧的结果文件
        if os.path.exists("result.csv"):
            try:
                os.remove("result.csv")
                print("🧹 已清理旧的 result.csv 文件，准备开始新测试...")
            except Exception as e:
                print(f"⚠️ 无法清理旧文件: {e}")
        ...
        # 3. 处理上传
        # 必须再次检查文件是否存在，且文件大小大于0（排除空文件）
        if os.path.exists("result.csv") and os.path.getsize("result.csv") > 0:
            # DNS 上传
            if args.zone_id and args.api_token and args.record_name:
                update_cloudflare_dns("result.csv", args.zone_id, args.api_token, args.record_name, args.upload_count)
            # GitHub 上传
            elif args.upload == 'github' and args.repo and args.token:
                upload_to_github_cli("result.csv", args.repo, args.token, args.file_path, args.upload_count)
        else:
            print("❌ 测速失败: 未生成有效的结果文件 (result.csv)")
            print("   可能原因: 网络不通(IPv6?)、IP文件无效、或没有满足条件的IP")
            return 1
    ...
    if args.mode == 'beginner':
        ...
        if os.path.exists("result.csv") and os.path.getsize("result.csv") > 0:
            print("\n✅ 测速完成！")
            if args.upload == 'api':
                if args.worker_domain and args.uuid:
        ...
        if not args.region:
            print("❌ 常规测速模式需要 --region 参数")
            return 1
        print("请保留原 normal 模式逻辑")

    elif args.mode == 'proxy':
        if not os.path.exists(args.csv):
            print(f"❌ 未找到CSV: {args.csv}")
            return 1
        generate_proxy_list(args.csv, "ips_ports.txt")
...
# ==========================================
# 新增功能：CloudFront 优选与 DNS 更新逻辑
# ==========================================

def update_cloudflare_dns(result_file, zone_id, api_token, record_name, upload_count=5):
    """更新 Cloudflare DNS 记录 (智能区分 A/AAAA，互不影响)"""
    print("\n" + "=" * 70)
    print(" Cloudflare DNS 自动更新")
    print("=" * 70)
    ...
    try:
        with open(result_file, 'r', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            for row in reader:
                ip = (row.get('IP 地址') or row.get('ip') or '').strip()
                if ip: best_ips.append(ip)
    except Exception as e:
        print(f"❌ 读取文件失败: {e}")
        return
    ...
    # 2. 关键修改：自动判断当前 IP 类型 (IPv4 vs IPv6)
    # 我们检查列表中的第一个 IP 来决定本次操作的目标类型
    sample_ip = best_ips[0]
    if ":" in sample_ip:
        target_type = "AAAA"
        print("ℹ️  检测到本次为 IPv6 优选，将只更新 AAAA 记录 (保留 A 记录)")
    else:
        target_type = "A"
        print("ℹ️  检测到本次为 IPv4 优选，将只更新 A 记录 (保留 AAAA 记录)")

    target_ips = best_ips[:upload_count]
    print(f"即将更新域名: {record_name}")
    print(f"选定最优 IP: {', '.join(target_ips)}")

    headers = {"Authorization": f"Bearer {api_token}", "Content-Type": "application/json"}
    
    # 3. 获取现有记录并删除 (仅针对特定类型)
    print(f"🔍 查询并清理旧的 {target_type} 记录...")
    try:
        # 关键修改：在 URL 中增加 &type={target_type} 参数
        # 这样 Cloudflare 只会返回同类型的记录，删除时绝对不会误删另一种类型
        list_url = f"https://api.cloudflare.com/client/v4/zones/{zone_id}/dns_records?name={record_name}&type={target_type}"
        
        try:
            resp = requests.get(list_url, headers=headers, timeout=15)
        except:
            resp = curl_request(list_url, headers=headers)
        ...
        if data.get('result'):
            for record in data['result']:
                ...
                try:
                    ...
                    curl_request(del_url, method='DELETE', headers=headers)
                print(f"  - 已删除旧记录: {record['content']} ({record['type']})")
                deleted_count += 1
        
        if deleted_count == 0:
            print(f"  (未发现旧的 {target_type} 记录，直接添加)")
            
    except Exception as e:
        print(f"⚠️ 查询/删除旧记录时出错: {e}")

    # 4. 添加新记录
    print(f"🚀 添加新的 {target_type} 记录...")
    for ip in target_ips:
        # 二次校验：确保混入的错误类型IP不会被上传
        current_ip_type = "AAAA" if ":" in ip else "A"
        if current_ip_type != target_type:
            print(f"  ⚠️ 跳过类型不匹配的 IP: {ip}")
            continue

        create_url = f"https://api.cloudflare.com/client/v4/zones/{zone_id}/dns_records"
        # type 字段强制使用我们判断出的 target_type
        payload = {"type": target_type, "name": record_name, "content": ip, "ttl": 60, "proxied": False}
        
        try:
            resp = requests.post(create_url, headers=headers, json=payload, timeout=15)
            res_json = resp.json()
        except:
            resp = curl_request(create_url, method='POST', data=payload, headers=headers)
            res_json = resp.json()

        if res_json.get('success'):
            print(f"  ✅ 添加成功: {ip}")
        else:
            err = res_json.get('errors', [{}])[0].get('message', '未知错误')
            print(f"  ❌ 添加失败: {ip} ({err})")
...
def select_optimization_mode():
    ...
    print(" 优选目标选择")
    print("=" * 60)
    print("  1. Cloudflare (CF)  - 优选 CF 边缘节点 (默认)")
    print("  2. CloudFront (CFT) - 优选 AWS CloudFront 节点")
    print("=" * 60)
    while True:
        choice = input("\n请选择优选目标 [1/2，默认: 1]: ").strip()
        if not choice or choice == "1":
            print("✓ 已选择: Cloudflare (CF)")
            return "cf"
        elif choice == "2":
            print("✓ 已选择: CloudFront (CFT)")
            return "cft"

# ==========================================
# CloudFront (CFT) 模块专用函数组
# ==========================================

def run_cft_process(exec_name, ip_file, url, dn_count, thread_count, speed_limit, time_limit, cli_ip_ver):
    """
    CFT 通用执行函数：负责运行测速、处理结果上报、生成命令行
    (修复：运行前清理 result.csv)
    """
    if sys.platform == "win32": cmd = [exec_name]
    else: cmd = [f"./{exec_name}"]
    ...
    if not os.path.exists("result.csv") or os.path.getsize("result.csv") == 0:
        print("\n❌ 测速失败或结果为空")
        print("   原因可能是：没有符合条件的IP、网络不支持IPv6、或IP文件无效")
        return
    ...
    print(" 优选结果处理")
    print("=" * 70)
    print("  1. 上传到 Cloudflare DNS (A/AAAA 记录)")
    print("  2. 上传到 GitHub (Gist/Repo)")
    print("  3. 不上传，直接结束")
    print("=" * 70)
    ...
    if upload_choice == "1":
        print("\n" + "=" * 60)
        print(" Cloudflare DNS 配置")
        print("=" * 60)
        
        saved_config = load_config() or {}
        saved_zone = saved_config.get('cft_zone_id')
        saved_token = saved_config.get('cft_api_token')
        saved_record = saved_config.get('cft_record_name')
        
        zone_id, api_token, record_name = None, None, None

        if saved_zone and saved_token and saved_record:
            print(f"\n检测到保存的配置: {saved_record}")
            if input("是否使用保存的配置？[Y/n]: ").strip().lower() not in ['n', 'no']:
                zone_id = saved_config.get('cft_zone_id')
                api_token = saved_config.get('cft_api_token')
                record_name = saved_config.get('cft_record_name')

        if not zone_id:
            print("\n请输入 Cloudflare API 信息:")
            zone_id = input("Zone ID: ").strip()
            api_token = input("API Token (需DNS编辑权限): ").strip()
            record_name = input("完整域名 (例如 cdn.example.com): ").strip()
        
        # 询问上传数量
        upload_count_input = input(f"请输入上传 IP 数量 [默认: 5]: ").strip() or "5"
        
        if zone_id and api_token and record_name:
            save_config(cft_zone_id=zone_id, cft_api_token=api_token, cft_record_name=record_name)
            update_cloudflare_dns("result.csv", zone_id, api_token, record_name, upload_count=int(upload_count_input))
            cli_upload_part = f"--zone-id {zone_id} --api-token {api_token} --record-name {record_name} --upload-count {upload_count_input}"

    elif upload_choice == "2":
        upload_info = upload_to_github("result.csv")
        if upload_info:
            cli_upload_part = f"--upload github --repo {upload_info['repo_info']} --token '{upload_info['github_token']}' --file-path {upload_info['file_path']} --upload-count {upload_info['upload_count']}"

    # 4. 生成并展示命令行指令
    script_path = os.path.abspath(sys.argv[0])
    python_exe = get_python_executable()
    
    full_cli_cmd = f"{python_exe} {script_path} --mode cloudfront {cli_ip_ver} --ip-file {ip_file} --url {url} --count {dn_count} --thread {thread_count} --speed {speed_limit} --delay {time_limit} {cli_upload_part}"
    full_cli_cmd = " ".join(full_cli_cmd.split())

    print("\n" + "=" * 80)
    print(" 💡 快速复用命令 (CLI)")
    print("=" * 80)
    print("您可以复制以下命令，用于下次直接运行或设置定时任务：\n")
    print(full_cli_cmd)
    print("\n" + "=" * 80)
    
    global LAST_GENERATED_COMMAND
    LAST_GENERATED_COMMAND = full_cli_cmd
    
def handle_cft_beginner_mode(exec_name):
    """CFT 小白快速测试模式 (升级版：包含5秒倒计时跳过下载)"""
    import time
    import sys
    ...
    print("  1. IPv4 (默认)")
    print("  2. IPv6")
    ip_choice = input("请输入 [1/2]: ").strip()
    
    CFT_IPV4_URL = "https://raw.githubusercontent.com/astemo6/yx-tools/refs/heads/main/CFT/ipv4.txt"
    CFT_IPV6_URL = "https://raw.githubusercontent.com/astemo6/yx-tools/refs/heads/main/CFT/ipv6.txt"
    
    if ip_choice == "2":
        target_url = CFT_IPV6_URL
        ip_file = "cft_ipv6.txt"
        cli_ip_ver = "--ipv6"
        print("✓ 已选择: IPv6")
    else:
        target_url = CFT_IPV4_URL
        ip_file = "cft_ipv4.txt"
        cli_ip_ver = ""
        print("✓ 已选择: IPv4")
    ...
    if os.path.exists(ip_file):
        print(f"✅ 发现本地文件: {ip_file}")
        print("是否重新下载最新列表？(输入 y 下载，其他键或等待则跳过)")
        ...
        try:
            if sys.platform == "win32":
                ...
                for i in range(timeout, 0, -1):
                    ...
                    # 在1秒内轮询检测按键
                    while time.time() - start_step < 1:
                        if msvcrt.kbhit():
                            # 获取输入字符
                            user_input = msvcrt.getwche()
                            break
                        time.sleep(0.05)
                    if user_input:
                        break
                ...
                # Linux/macOS 倒计时逻辑
                import select
                print(f"⏳ 请在 {timeout} 秒内输入 y 并回车 (否则自动跳过): ", end="", flush=True)
        ...
        # 判断用户输入
        if user_input and str(user_input).lower().strip() == 'y':
            print("\n🔄 开始下载更新...")
            need_download = True
        else:
            print("\n⏩ 跳过下载，使用本地文件")
    ...
    # 执行下载
    if need_download:
        download_file(target_url, ip_file)
    
    # 最终确认文件存在
    if not os.path.exists(ip_file):
        print(f"❌ 错误：无法获取 IP 文件，请检查网络或手动上传 {ip_file}")
        return

    # 使用 NASA 8K 视频作为默认链接
    default_url = "https://images-assets.nasa.gov/video/First-8K-Video-from-Space/First-8K-Video-from-Space~orig.mp4"
    ...
    # 第一步：设置测试IP数量
    print("\n📊 第一步：设置测试IP数量")
    dn_count = input("请输入要测试的IP数量 [默认: 10]: ").strip() or "10"

    # 第二步：设置延迟上限
    print(f"\n⏱️  第二步：设置延迟上限")
    time_limit = input("请输入延迟上限(ms) [默认: 1000]: ").strip() or "1000"

    # 第三步：设置下载速度下限
    print(f"\n🚀 第三步：设置下载速度下限")
    speed_limit = input("请输入下载速度下限(MB/s) [默认: 1]: ").strip() or "1"

    # 第四步：设置延迟测速线程数
    print(f"\n⚡ 第四步：设置延迟测速线程数")
    thread_count = input("请输入延迟测速线程数 [默认: 200, 最多: 1000]: ").strip() or "200"

    # 执行通用逻辑
    run_cft_process(exec_name, ip_file, default_url, dn_count, thread_count, speed_limit, time_limit, cli_ip_ver)

def handle_cft_normal_mode(exec_name):
    """CFT 进阶自定义模式"""
    print("\n" + "=" * 70)
    print(" CFT - 进阶自定义模式")
    print("=" * 70)
    print(" 说明：完全自定义 IP 文件、测速链接及所有参数。")
    
    # 1. IP 版本选择
    print("\n请选择 IP 版本 (用于生成正确的 DNS 记录):")
    print("  1. IPv4 (默认)")
    print("  2. IPv6")
    ip_choice = input("请输入 [1/2]: ").strip()
    ...
    if ip_choice == "2":
        cli_ip_ver = "--ipv6"
        default_file = "cft_ipv6.txt"
        print("✓ 已选择: IPv6")
    else:
        cli_ip_ver = ""
        default_file = "cft_ipv4.txt"
        print("✓ 已选择: IPv4")

    # 2. 文件路径
    ip_file = input(f"\n请输入 IP 文件路径 [默认: {default_file}]: ").strip() or default_file
    if not os.path.exists(ip_file):
        print(f"⚠️  文件 {ip_file} 不存在，尝试自动下载默认列表...")
        # 尝试下载兜底
        if "ipv6" in default_file:
            download_file("https://raw.githubusercontent.com/astemo6/yx-tools/refs/heads/main/CFT/ipv6.txt", ip_file)
        else:
            download_file("https://raw.githubusercontent.com/astemo6/yx-tools/refs/heads/main/CFT/ipv4.txt", ip_file)
    ...
    # 3. 自定义 URL
    default_url = "https://images-assets.nasa.gov/video/First-8K-Video-from-Space/First-8K-Video-from-Space~orig.mp4"
    url = input(f"\n请输入测速 URL [默认: NASA 8K]: ").strip()
    if not url: url = default_url

    # 4. 详细参数 (现在也加入延迟设置)
    print(f"\n⚡ 设置详细参数")
    dn_count = input("测试数量 [10]: ").strip() or "10"
    time_limit = input("延迟上限(ms) [1000]: ").strip() or "1000" # 新增
    speed_limit = input("速度下限(MB/s) [0]: ").strip() or "0" 
    thread_count = input("线程数 [200]: ").strip() or "200"
    
    # 执行通用逻辑
    run_cft_process(exec_name, ip_file, url, dn_count, thread_count, speed_limit, time_limit, cli_ip_ver)

def handle_cloudfront_mode(exec_name):
    """CloudFront 模式入口菜单"""
    print("\n" + "=" * 70)
    print(" CloudFront (CFT) 优选模式")
    print("=" * 70)
    print("  1. 小白快速测试 (自动下载 IP，使用 NASA 8K 链接)")
    print("  2. 进阶自定义测试 (自定义 IP 文件、URL 及参数)")
    print("=" * 70)
    
    choice = input("\n请选择功能 [默认: 1]: ").strip()
    
    if choice == "2":
        handle_cft_normal_mode(exec_name)
    else:
        handle_cft_beginner_mode(exec_name)
...
def main():
    ...
    # 这里是你插入的新代码
    # ==========================
    target_mode = select_optimization_mode()
    
    if target_mode == "cft":
        # === 进入 CloudFront 流程 ===
        handle_cloudfront_mode(exec_name)
        print("\nCloudFront 优选流程结束")
        # 注意：这里删除了 return 0，代码会继续向下运行去设置定时任务

    else:
        # === 否则进入 Cloudflare 流程 (下面的代码都需要缩进) ===
...
def upload_to_github(result_file="result.csv"):
    ...
    try:
        ...
        content_lines = []
       	...
       	# --- 修改开始：自动判断 V4/V6 并添加序号 ---
        for i, ip_info in enumerate(best_ips[:upload_count], 1):
            # 获取地区名称
            region_name = ip_info.get('region_name', '未知地区')
            
            # 1. 判断 IP 版本 (通过是否包含冒号)
            is_ipv6 = ":" in ip_info['ip']
            
            # 2. 设置后缀：如果是 IPv6 用 "V6"，否则用 "V4"
            version_suffix = "V6" if is_ipv6 else "V4"
            
            # 3. 构建名称：地区 + 版本后缀 + 横杠 + 两位序号
            # 例如: 新加坡V4-01 或 新加坡V6-01
            name = f"{region_name}{version_suffix}-{i:02d}"
            
            # 4. 格式化输出 (IPv6 地址需要加中括号 [])
            if is_ipv6:
